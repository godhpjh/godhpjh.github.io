...
WITH CCS_SET AS
	...
	SELECT X.*
	  FROM (
			/* 동일 종목간 괴리율 */
			SELECT CASE WHEN NVL(DPRT_RT_SET.DPRT_RT,0) > #{dprtRt, jdbcType=NUMERIC} THEN '1.위반'
						ELSE '2.정상'
						 END AS CAL_STATUS
				  ,CCS_SET.ORD_DT
				  ,USR_BSC_FNMS.TEM_CD
				  ...
				  ,NVL(DPRT_RT_SET.DPRT_RT,0) AS DPRT_RT    /* 괴리율 */
				  ,BD_ITM.PUB_DT                            /* 발행일자 */
				  	...
				  	SELECT SOURCE_ISIN AS ISIN_CD
				  	  ...
				  	  FROM(
				  			SELECT A.ISIN_CD AS SOURCE_ISIN
				  				  ...
				  				  ,ABS(NVL(A.CCS_ROR,0) - NVL(B.CCS_ROR,0)) AS DPRT_RT
				  			  FROM ${SCHEMA_NAME}.TSOL_OM_INTG_BKR_ORD_ITM_TR_SFS A
				  				  ,${SCHEMA_NAME}.TSOL_OM_INTG_BKR_ORD_ITM_TR_SFS B
				  			 WHERE 1=1
			 ...
			 WHERE 1=1
			   ...
			   /* 동일종목 괴리율 */
			   AND CCS_SET.ISIN_CD = DPRT_RT_SET.ISIN_CD (+)
		    ) X
