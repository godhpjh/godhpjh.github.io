...
WITH CCS_SET AS
	...
	SELECT FND_CCS_ALCN.FND_CD
		  ...
		  ,SUM(NVL_NOA(BD_CCS_BSC.STM_AMT,0))     AS STM_AMT /* 결제금액 */
		  ,MAX(NVL_NOA(BKR_ITM_TR.CCS_ROR,0))     AS CCS_ROR /*매매이율*/
	  FROM ${SCHEMA_NAME}.TSOL_OM_INTG_ORD_FND_CCS_ALCN_SFS FND_CCS_ALCN /* EOM_통합주문펀드체결배분명세 */
		   INNER JOIN ${SCHEMA_NAME}.TSOL_OM_BD_CCS_BSC BD_CCS_BSC /* EOM_채권체결기본 */
		   ON(/* EOM_채권체결기본 */
	           FND_CCS_ALCN.IST_CD = BD_CCS_BSC.IST_CD
		   ...
		   ON(/* EOM_통합브로커주문종목거래명세 */
	           FND_CCS_ALCN.IST_CD = BKR_ITM_TR.IST_CD
		   ...
		   ON(/* EOM_통합브로커주문거래기본 */
	           FND_CCS_ALCN.IST_CD = BKR_ORD_TR.IST_CD
		   ...
		   ON(/* EOM_통합주문진행상황명세 */
	           FND_CCS_ALCN.IST_CD = PRG_STA.IST_CD
	...
	SELECT X.*
	  FROM (
			/* 동일 종목간 괴리율 */
			SELECT CASE WHEN NVL_NOA(DPRT_RT_SET.DPRT_RT,0) > #{dprtRt, jdbcType=NUMERIC} THEN '1.위반'
						ELSE '2.정상'
						 END AS CAL_STATUS
				  ,CCS_SET.ORD_DT
				  ,USR_BSC_FNMS.TEM_CD
				  ...
				  ,NVL_NOA(BD_GR.LST_YTM,0) * 100   AS EVL_ERN_RT /* 평가수익율 */
				  ,NVL_NOA(DPRT_RT_SET.DPRT_RT,0) AS DPRT_RT    /* 괴리율 */
				  ,BD_ITM.PUB_DT                            /* 발행일자 */
			  ...
			  FROM CCS_SET
				   ...
				   ON (/* ECP_채권불공정사유명세 */
			           CCS_SET.ORNO = UNFR_SFS.ORNO
			       AND CCS_SET.FND_CD = UNFR_SFS.FND_CD
			       AND CCS_SET.ISIN_CD = UNFR_SFS.ISIN_CD
			       AND CCS_SET.FNMN_USID = UNFR_SFS.FNMN_USID
			       AND UNFR_SFS.CKG_HDN_CD = 'D')

				   LEFT OUTER JOIN ${SCHEMA_NAME}.TSOL_DM_BD_GR_SFS BD_GR /* EDM_채권등급명세 */
				   ON (/* EDM_채권등급명세 */
			           #{istCd, jdbcType=VARCHAR} = BD_GR.IST_CD
			       AND F_BDD_SLS_DT_NOA(#{ordDt, jdbcType=VARCHAR}, 1) = BD_GR.BSE_DT
			       AND CCS_SET.ISIN_CD = BD_GR.ISIN_CD)
				   
				   INNER JOIN ${SCHEMA_NAME}.TSOL_DM_BD_ITM_SFS BD_ITM  /* EDM_채권종목명세 */
				   ON (/* EDM_채권종목명세 */
			           #{istCd, jdbcType=VARCHAR} = BD_ITM.IST_CD
			       AND CCS_SET.ISIN_CD = BD_ITM.ISIN_CD)

				   LEFT OUTER JOIN (
				  	    SELECT SOURCE_ISIN AS ISIN_CD
				  		      ,MAX(DPRT_RT) AS DPRT_RT
				  	      FROM (
				  			    SELECT A.ISIN_CD AS SOURCE_ISIN
				  				      ,B.ISIN_CD AS TARGET_ISIN
				  				      ,ABS(NVL_NOA(A.CCS_ROR,0) - NVL_NOA(B.CCS_ROR,0)) AS DPRT_RT
				  			      FROM ${SCHEMA_NAME}.TSOL_OM_INTG_BKR_ORD_ITM_TR_SFS A
				  				       INNER JOIN ${SCHEMA_NAME}.TSOL_OM_INTG_BKR_ORD_ITM_TR_SFS B
									   ON (/* */
									       A.IST_CD = B.IST_CD
				  			           ...
				  			           AND A.TR_CD = B.TR_CD
				  			           <![CDATA[
				  			           AND A.BKR_ITM_ORNO <> B.BKR_ITM_ORNO
				  			           ]]>)
				  			     WHERE 1=1
				  			       AND A.IST_CD = #{istCd, jdbcType=VARCHAR}
				  	     ...
				  	     WHERE SOURCE_ISIN = TARGET_ISIN
				  	       GROUP BY SOURCE_ISIN
				        ) DPRT_RT_SET
				   ON (/* 동일종목 괴리율 */
			           CCS_SET.ISIN_CD = DPRT_RT_SET.ISIN_CD)
		    ) X
