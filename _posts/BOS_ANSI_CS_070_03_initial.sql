...
WITH CCS_SET AS
	...
	SELECT FND_CCS_ALCN.FND_CD
	  ...
	  FROM ${SCHEMA_NAME}.TSOL_OM_INTG_ORD_FND_CCS_ALCN_SFS FND_CCS_ALCN   /* EOM_통합주문펀드체결배분명세 */
		  ...
		  ,${SCHEMA_NAME}.TSOL_OM_INTG_ORD_CMPL_APV_SFS CMPL_APV            /* EOM_통합주문컴플승인명세 */
	 WHERE 1=1
	   /* EOM_통합주문펀드체결배분명세 */
	   ...
	   AND BKR_ORD_ITM.FST_CCS_DTM BETWEEN TO_DATE(#{ccuAsprSttHr, jdbcType=VARCHAR}, 'YYYY-MM-DD HH24:MI:SS') AND TO_DATE(#{ccuAsprEndHr, jdbcType=VARCHAR}, 'YYYY-MM-DD HH24:MI:SS') /*동시호가시간*/
	...
	SELECT FND_CCS_ALCN.FND_CD
	 ...
	 WHERE 1=1
	   /* EOM_통합주문펀드체결배분명세 */
	   AND FND_CCS_ALCN.IST_CD = #{istCd, jdbcType=VARCHAR}
	   AND FND_CCS_ALCN.ORD_DT = F_BDD_SLS_DT(#{ordDt, jdbcType=VARCHAR} ,1) /* T-1 영업일 */
	   AND FND_CCS_ALCN.AMCO_CD = #{amcoCd, jdbcType=VARCHAR}
	   AND FND_CCS_ALCN.TR_CD = '101'
	   AND FND_CCS_ALCN.CCS_QTY > 0
	   AND FND_CCS_ALCN.DEL_YN = 'N'
	   ...
	   AND FND_CCS_ALCN.TR_CD = BKR_ORD_ITM.TR_CD
	   AND FND_CCS_ALCN.BKR_ITM_ORNO = BKR_ORD_ITM.BKR_ITM_ORNO
	   AND BKR_ORD_ITM.FST_CCS_DTM BETWEEN TO_DATE(#{t1CcuAsprSttHr, jdbcType=VARCHAR}, 'YYYY-MM-DD HH24:MI:SS') AND TO_DATE(#{t1CcuAsprEndHr, jdbcType=VARCHAR}, 'YYYY-MM-DD HH24:MI:SS') /*T-1 동시호가시간*/
				...
				SELECT SUM(CASE WHEN CAL_RT > 1 THEN 1 ELSE 0 END) AS VLT_CNT
				  ...
				  FROM (
						SELECT CCS_SET.FND_CD
							  ,FND_BSC.FND_NM
							  ,CASE WHEN CCS_SET.ORD_DT = F_BDD_SLS_DT(#{ordDt, jdbcType=VARCHAR},1) THEN 'T-1'
									ELSE 'T'
									 END AS GUBUN
							  ,CCS_SET.ISIN_CD
							  ,CCS_SET.ITM_NM
							  ...
							  ,CASE WHEN NVL(ITM_AVR.D20_CCS_ASPR_VOLE,0) = 0 THEN 0
						 ...
						 WHERE 1=1
						   ...
						   AND ITM_AVR.BSE_DT = F_NDD_SLS_DT(CCS_SET.ORD_DT, 1)
