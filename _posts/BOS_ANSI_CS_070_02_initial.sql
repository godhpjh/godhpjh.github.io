...
WITH CCS_SET AS
	...
	SELECT FND_CCS_ALCN.FND_CD
	  ...
	  FROM ${SCHEMA_NAME}.TSOL_OM_INTG_ORD_FND_CCS_ALCN_SFS FND_CCS_ALCN   /* EOM_통합주문펀드체결배분명세 */
		  ,${SCHEMA_NAME}.TSOL_OM_INTG_BKR_ORD_ITM_TR_SFS BKR_ORD_ITM       /* EOM_통합브로커주문종목거래명세 */
		  ,${SCHEMA_NAME}.TSOL_OM_INTG_ORD_ITM_TR_SFS ITM_TR                /* EOM_통합주문종목거래명세 */
		  ,${SCHEMA_NAME}.TSOL_CP_BFR_CKG_SAF CMPL_APV            /* ECP_사전점검현황 */
	 WHERE 1=1
	   /* EOM_통합주문펀드체결배분명세 */
	   ...
	   /* EOM_통합브로커주문종목거래명세 */
	   AND FND_CCS_ALCN.IST_CD = BKR_ORD_ITM.IST_CD
	   ...
	   AND BKR_ORD_ITM.FST_CCS_DTM BETWEEN TO_DATE(#{ccuAsprSttHr, jdbcType=VARCHAR}, 'YYYY-MM-DD HH24:MI:SS') AND TO_DATE(#{ccuAsprEndHr, jdbcType=VARCHAR}, 'YYYY-MM-DD HH24:MI:SS')

	   /* EOM_통합주문컴플승인명세 */
	   AND FND_CCS_ALCN.IST_CD = CMPL_APV.IST_CD
	   ...
	   AND NOT EXISTS(
						SELECT 1
						  FROM ${SCHEMA_NAME}.TSOL_OM_INTG_ORD_ITM_TR_SFS A
							  ,${SCHEMA_NAME}.TSOL_OM_INTG_ORD_PRG_STA_SFS B
						 WHERE A.IST_CD = #{istCd, jdbcType=VARCHAR}
						   ...
						   AND A.TR_CD = B.TR_CD
						   AND A.ORNO = B.ORNO
						   AND B.ORD_GNT_DTM <= TO_DATE(#{mktEndHr, jdbcType=VARCHAR}, 'YYYY-MM-DD HH24:MI:SS')
					 )
	...
	SELECT X.*
	  FROM (
			SELECT CASE WHEN (
								CASE WHEN NVL(ITM_AVR.D20_AVR_VOLE,0) = 0 THEN 0
			  ...
			  FROM CCS_SET
				  ...
				  ,${SCHEMA_NAME}.TSOL_FW_DPM_CD_CTL FNMS_DPM_CD_CTL      /* 운용역부서코드  */
				  ,${SCHEMA_NAME}.TSOL_FW_DPM_CD_CTL FNMS_HRK_DPM_CD_CTL  /* 운용역본부코드 */
				  ,${SCHEMA_NAME}.TSOL_DM_STK_ITM_AVR_VOLE_SFS ITM_AVR    /*EDM_주식종목평균거래량명세*/
			 WHERE 1=1
			   ...
			   /* 운용역부서코드 */
			   AND #{istCd, jdbcType=VARCHAR} = FNMS_DPM_CD_CTL.IST_CD (+)
			   AND USR_BSC_FNMS.DPM_CD = FNMS_DPM_CD_CTL.DPM_CD (+)

			   /* 운용역본부코드 */
			   AND #{istCd, jdbcType=VARCHAR} = FNMS_HRK_DPM_CD_CTL.IST_CD (+)
			   AND FNMS_DPM_CD_CTL.HRK_DPM_ID = FNMS_HRK_DPM_CD_CTL.DPM_CD (+)

			   /*20일평균시장거래량*/
			   AND ITM_AVR.IST_CD = #{istCd, jdbcType=VARCHAR}
