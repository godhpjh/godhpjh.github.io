...
WITH CCS_SET AS
	(
		SELECT BD_CCS_BSC.FND_CD
			  ...
			  ,SUM(NVL(BD_CCS_BSC.ORD_PAR_AMT, 0)) AS ORD_QTY /* 주문수량(액면)  */
			  ,SUM(NVL(BD_CCS_BSC.CCS_PAR_AMT, 0)) AS CCS_QTY /* 체결금액  */
			  ,MAX(NVL(BD_CCS_BSC.CCS_UPR, 0)) AS CCS_UPR /* 체결단가 */
			  ,MAX(NVL(BD_CCS_BSC.DLN_ROR, 0)) AS CCS_ROR /*매매이율*/
		  FROM ${SCHEMA_NAME}.TSOL_OM_BD_CCS_BSC BD_CCS_BSC /* EOM_채권체결기본 */
			  ...
			  ,${SCHEMA_NAME}.TSOL_DM_BD_ITM_SFS BD_ITM
		 WHERE 1 = 1
		   /* EOM_채권체결기본 */
		   ...
		   /* EOM_통합브로커주문종목거래명세 */
		   AND BD_CCS_BSC.IST_CD = BKR_ITM_TR.IST_CD
		   ...
		   /* EOM_통합브로커주문거래기본 */
		   AND BKR_ITM_TR.IST_CD = BKR_ORD_TR.IST_CD
		   ...
		   /* EOM_통합주문진행상황명세 */
		   AND BD_CCS_BSC.IST_CD = PRG_STA.IST_CD
		   ...
		   AND BD_CCS_BSC.IST_CD = BD_ITM.IST_CD
	...
	SELECT CCS_SET.ISIN_CD             AS ITM_CD
		  ...
		  ,TO_CHAR(CCS_SET.FNMN_HDER_APV_DTM, 'HH24:MI:SS') AS ORD_HR /*주문시간*/
		  ,TO_CHAR(CCS_SET.FST_CCS_DTM, 'HH24:MI:SS')       AS CCS_HR /*체결시간*/
	  FROM CCS_SET
		  ,${SCHEMA_NAME}.TSOL_OM_FND_BSC FND_BSC
		  ,${SCHEMA_NAME}.TSOL_FW_USR_BSC USR_BSC_FNMS      /* 사용자기본 운용역 */
		  ,${SCHEMA_NAME}.TSOL_FW_USR_BSC USR_BSC_TRR		 /*사용자기본 트레이더*/
	...
	WHERE 1=1
	  ...
	  /* 트레이더명 */
	  AND #{istCd, jdbcType=VARCHAR} = USR_BSC_TRR.IST_CD (+)
	  ...
	  AND TO_CHAR(TO_DATE(CCS_SET.ORD_DT,'YYYYMMDD')-1,'YYYYMMDD') = BD_GR.BSE_DT (+)
