...
WITH CCS_SET AS
	...
	SELECT X.*
	  FROM (
			/* 평가수익율 대비 괴리율 */
			SELECT CASE WHEN ABS(NVL_NOA(CCS_SET.CCS_ROR,0) - (NVL_NOA(BD_GR.LST_YTM,0) * 100)) > #{dprtRt, jdbcType=NUMERIC} THEN '1.위반'
						ELSE '2.정상'
						 END AS CAL_STATUS
				  ,CCS_SET.ORD_DT
				  ,USR_BSC_FNMS.TEM_CD
				  ...
				  ,ABS(NVL_NOA(CCS_SET.CCS_ROR,0) - (NVL_NOA(BD_GR.LST_YTM,0) * 100)) AS DPRT_RT /*괴리율*/
				  ,BD_ITM.PUB_DT                              /* 발행일자 */
