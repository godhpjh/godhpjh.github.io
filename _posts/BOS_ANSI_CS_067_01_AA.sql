...
WITH CCS_SET AS
      (
        SELECT
		  ...
		  , CASE WHEN (TO_CHAR_NOA(MIN(PRG_STA.FNMN_HDER_APV_DTM), 'HH24:MI:SS') > TO_CHAR_NOA(MIN(BKR_ORD_TR.TRR_SHR_DTM), 'HH24:MI:SS')
                       OR
                       TO_CHAR_NOA(MIN(BKR_ORD_TR.TRR_SHR_DTM), 'HH24:MI:SS') > TO_CHAR_NOA(BD_CCS_BSC.FST_CCS_DTM, 'HH24:MI:SS')
                       OR
                       TO_CHAR_NOA(MIN(PRG_STA.FNMN_HDER_APV_DTM), 'HH24:MI:SS') > TO_CHAR_NOA(BD_CCS_BSC.FST_CCS_DTM, 'HH24:MI:SS')
                      ) THEN 'Y'
            ELSE 'N'
        ...
        FROM ${SCHEMA_NAME}.TSOL_OM_INTG_ORD_FND_CCS_ALCN_SFS FND_CCS_ALCN       /* EOM_통합주문펀드체결배분명세 */
          ...
          INNER JOIN ${SCHEMA_NAME}.TSOL_OM_INTG_ORD_TR_BSC ORD_TR               /* EOM_통합주문거래기본 */
          ON (/* EOM_통합주문거래기본 */
              FND_CCS_ALCN.IST_CD = ORD_TR.IST_CD
          ...
          LEFT OUTER JOIN ${SCHEMA_NAME}.TSOL_OM_BD_CCS_BSC BD_CCS_BSC           /* EOM_채권체결기본 */
          ON (/* EOM_채권체결기본 */
        ...
        WHERE 1=1
          ...
          AND EXISTS (
				      SELECT 1
				        FROM F_GET_ATH_FND_NOA(#{istCd,  jdbcType=VARCHAR}, #{amcoCd, jdbcType=VARCHAR}, #{usid, jdbcType=VARCHAR}, '', '', '', '', '', '') ATH
      ...
      SELECT
        ...
        , CCS_SET.SRT_VLT
        , CCS_SET.TR_PCDR_VLT
        , TO_CHAR_NOA(CCS_SET.ORD_APV_REQ_DTM, 'HH24:MI:SS')   AS ORD_APV_REQ_HR     /* 주문승인요청시간 */
        , CCS_SET.FNMN_HDER_USID                           AS ORD_APV_USR_NM     /* 주문승인자ID */
        , CCS_SET.FNMN_HDER_APV_TC                         AS ORD_APV_YN         /* 운용역헤더승인구분 */
        , TO_CHAR_NOA(CCS_SET.FNMN_HDER_APV_DTM, 'HH24:MI:SS') AS ORD_APV_HR         /* 운용역헤더승인일시 */
        , CCS_SET.FNMN_HDER_USR_IP                         AS ORD_APV_IP         /* 운용역헤더승인IP */

        , CCS_SET.TRR_USID                                 AS SHR_USR_NM         /* 트레이더배정자 */
        , TO_CHAR_NOA(CCS_SET.TRR_SHR_DTM, 'HH24:MI:SS')       AS SHR_HR             /* 트레이더배분일시 */
        , CCS_SET.TRR_USR_IP                               AS SHR_IP             /* 매매실행IP */
        , TO_CHAR_NOA(CCS_SET.FST_CCS_DTM, 'HH24:MI:SS')       AS CCS_HR             /* 최초체결일시 */
        , CCS_SET.BKR_CD
      ...
      FROM CCS_SET
           ...
           LEFT OUTER JOIN ${SCHEMA_NAME}.TSOL_OM_AST_TR_CD_CTL AST_TR_CD /* 거래코드  */
           ON (/* 거래코드  */
               #{istCd,jdbcType=VARCHAR} = AST_TR_CD.IST_CD
